    ...
    <template #default>
      <el-space wrap size="large">
        <el-input
          ...
          v-model="inputRefs[column.key]"
          :disabled="inputDisableRefs[column.key]"
...
import { Loading as LoadingIcon, Timer } from '@element-plus/icons-vue'
      ...
      columns.value = dicKeys.map((dictkey) => {
        ...
        if (dictkey.includes('date')) {
          columnDefinition.cellRenderer = (date) => (
            <ElTooltip
              content={dayjs(date.cellData).format('YYYY-MM-DD HH:mm')}
            >
              <div>
                ...
                <span class="items-center">
                  {' '}
                  {dayjs(date.cellData).format('YYYY-MM-DD HH:mm')}{' '}
                </span>
              </div>
            </ElTooltip>
          )
        ...
        if (dictkey === 'id') {
          columnDefinition.cellRenderer = (ID) => <b>{ID.cellData}</b>
        }
